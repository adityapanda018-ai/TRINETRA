/**
 * TRINETRA — upstream source cache.
 *
    ...
    const inflight = (async () => {
      try {
        ...
        if (entry?.data.length) {
          console.warn(`[TRINETRA] ${key} refresh failed — serving ${entry.data.length} cached cameras`);
        ...
        console.warn(`[TRINETRA] ${key} fetch failed with no cache to fall back on:`, e);
