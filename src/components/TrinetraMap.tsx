...
interface TrinetraMapProps {
  ...
  /** Live 360-degree perimeter sentry defense rings around user location. */
  sentryMode?: boolean;
  /** Tactical optics mode: normal, NVG green phosphor, FLIR thermal, or 1983 CRT. */
  tacticalOptics?: 'normal' | 'nvg' | 'flir' | 'crt';
...
function TrinetraMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false, aircraftAirports = {}, sentryMode = false, tacticalOptics = 'normal', is3dTerrain = false, onToggle3dTerrain, radarDome = null }: TrinetraMapProps) {
  ...
  const camerasMapRef = useRef<Map<string, any>>(new Map());
  ...
  useEffect(() => {
    ...
    for (const canvasContextAttributes of attributeFallbacks) {
      try {
        ...
        console.warn('[TRINETRA] WebGL context rejected, retrying with weaker attributes:', e instanceof Error ? e.message : e);
      }
    }
    if (!map) return;

    // Guard queryRenderedFeatures against MapLibre GL tile re-indexing race conditions
    // (e.g. TypeError: Cannot read properties of undefined (reading 'feature') in loadMatchingFeature)
    const origQueryRenderedFeatures = map.queryRenderedFeatures.bind(map);
    map.queryRenderedFeatures = function (geometryOrOptions?: any, options?: any) {
      try {
        return origQueryRenderedFeatures(geometryOrOptions, options);
      } catch (err: any) {
        if (err instanceof TypeError && (err.message?.includes('feature') || err.message?.includes('undefined'))) {
          return [];
        }
        return [];
      }
    ...
    // WebGL Context Loss & Auto-Recovery Guard (laptop sleep, GPU driver reset, etc.)
    const canvas = map.getCanvas();
    const onContextLost = (e: Event) => {
      e.preventDefault();
      console.warn('[TRINETRA] WebGL context lost — awaiting GPU restoration');
    };
    const onContextRestored = () => {
      console.log('[TRINETRA] WebGL context restored — resizing map canvas');
      try {
        map?.resize();
        map?.triggerRepaint();
      } catch (err) {
        console.error('[TRINETRA] Map recovery failed after WebGL restoration:', err);
      }
    };
    ...
    map.on('load', () => {
      ...
      // Create icons — TRINETRA Unified Palette
      ...
      sources.forEach(s => {
        if (s === 'cctv') {
          map.addSource(s, { type: 'geojson', data: EMPTY_FC, buffer: 0, tolerance: 1.5 });
        } else {
      ...
      // ══ TRINETRA SDK — Lattice Intelligence Mesh ══
      ...
      if (process.env.NODE_ENV === 'development') (window as any).__trinetraMap = map; (window as any).__osirisMap = map;
    });
    ...
    ['fl-commercial','fl-private','fl-jets','fl-military'].forEach(layer => {
      map.on('click', layer, (e: any) => {
        ...
        popup(coords, `<div style="${pStyle}border:1px solid rgba(255,255,255,0.08);">
          ...
          <button onclick="(window.trinetraWatchFlight || window.osirisWatchFlight) && (window.trinetraWatchFlight || window.osirisWatchFlight)({ icao24: '${idSafe(p.icao24||'')}', callsign: '${idSafe(cs)}' })" style="width:100%;margin-top:8px;padding:6px 12px;background:rgba(0,229,255,0.10);border:1px solid rgba(0,229,255,0.35);color:#7FE9FF;font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:bold;letter-spacing:0.1em;border-radius:4px;cursor:pointer;">+ WATCH THIS AIRCRAFT</button>
    ...
    map.on('click', 'cctv-dots', (e: any) => {
      ...
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const cam = camerasMapRef.current.get(p.id) || p;
    ...
    map.on('click', (e: any) => {
      ...
      let hits: any[] = [];
      try {
        const queryLayers = Array.from(CLICKABLE_LAYERS).filter(id => map.getLayer(id));
        hits = queryLayers.length > 0 ? map.queryRenderedFeatures(e.point, { layers: queryLayers }) : [];
      } catch {
        hits = [];
      }
    ...
    map.on('mousemove', (e: any) => {
      ...
      if (now - lastSatHover < 100) return; // 100ms throttle — stops 19k-point offscreen pass on every mouse move!
      lastSatHover = now;
      hoverQueued = true;
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, (e: any) => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || 'https://trinetra.live';
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${urlSafe(srcUrl)}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${htmlEsc(p.source || 'TRINETRA')}</a></div>
          </div>
  ...
  useEffect(() => {
    if (!mapReady) return;
    if (!activeLayers.cctv || !data.cameras) {
      setGeo('cctv', []);
      return;
    }
    const camMap = new Map<string, any>();
    const features = new Array(data.cameras.length);
    for (let i = 0; i < data.cameras.length; i++) {
      const c = data.cameras[i];
      camMap.set(c.id, c);
      features[i] = {
        type: 'Feature',
  ...
  // ══ TRINETRA SDK — Lattice Sensor Mesh ══
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('trinetra-buildings')) {
          map.addSource('trinetra-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('trinetra-3d-buildings')) {
          map.addLayer({
            id: 'trinetra-3d-buildings',
            source: 'trinetra-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('trinetra-3d-buildings')) map.removeLayer('trinetra-3d-buildings');
      }
    } catch (e) {
      console.warn('[TRINETRA] 3D terrain toggle error:', e);
    }
  ...
  // ── 360° SENTRY PERIMETER RINGS ──
  useEffect(() => {
    if (!mapReady || !mapRef.current) return;
    const map = mapRef.current;
    const SRC = 'sentry-rings-source';
    const IDS = ['sentry-rings-fill', 'sentry-rings-line'];

    if (!sentryMode || !userLocation) {
      IDS.forEach(id => { if (map.getLayer(id)) map.removeLayer(id); });
    ...
    const ring75 = circleToRing(center, 75, 64);
    const ring25 = circleToRing(center, 25, 64);
    const ring5 = circleToRing(center, 5, 64);
    ...
    const fc: GeoJSON.FeatureCollection = {
      ...
      features: [
        {
          type: 'Feature',
          properties: { zone: 'surveillance', color: '#00E676' },
          geometry: { type: 'Polygon', coordinates: [ring75] },
        },
        {
          type: 'Feature',
          properties: { zone: 'advisory', color: '#FFD700' },
          geometry: { type: 'Polygon', coordinates: [ring25] },
        },
        {
          type: 'Feature',
          properties: { zone: 'defense', color: '#FF3D3D' },
          geometry: { type: 'Polygon', coordinates: [ring5] },
        },
      ],
    };
    ...
    if (!map.getSource(SRC)) {
      ...
      map.addLayer({
        id: 'sentry-rings-fill',
        type: 'fill',
        source: SRC,
        paint: {
          'fill-color': ['get', 'color'],
          'fill-opacity': ['match', ['get', 'zone'], 'defense', 0.08, 'advisory', 0.04, 0.02],
        },
      });
      map.addLayer({
        id: 'sentry-rings-line',
        type: 'line',
        source: SRC,
        paint: {
          'line-color': ['get', 'color'],
          'line-width': ['match', ['get', 'zone'], 'defense', 2, 'advisory', 1.5, 1],
          'line-dasharray': [4, 2],
          'line-opacity': 0.8,
        },
      });
    } else {
      (map.getSource(SRC) as maplibregl.GeoJSONSource).setData(fc as never);
    }
  }, [mapReady, sentryMode, userLocation]);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (now - lastPulse >= 50) { // ~20 FPS throttle
        lastPulse = now;
        if (map.getLayer('user-dot-pulse')) {
  ...
  // Escape clears the selection, the way it cancels a draw
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[TRINETRA] 3D terrain switch error:', err);
    }
  }, [mapReady, is3dTerrain]);
  ...
  useEffect(() => {
    ...
    const sourceId = 'trinetra-radar-dome-source';
    const fillLayerId = 'trinetra-radar-dome-fill';
    const lineLayerId = 'trinetra-radar-dome-line';
    const centerLayerId = 'trinetra-radar-dome-center';
    ...
    const featureCollection = {
      ...
      features: [
        {
          type: 'Feature',
          geometry: radarDome.geometry,
          properties: { label: radarDome.label }
        },
        {
          type: 'Feature',
          geometry: {
            type: 'Point',
            coordinates: radarDome.center
          },
          properties: { isCenter: true, label: radarDome.label }
        }
      ]
    };
...
export { TrinetraMap, TrinetraMap as OsirisMap };
export default memo(TrinetraMap);
