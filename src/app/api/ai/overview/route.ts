/**
 * ═══════════════════════════════════════════════════════════════
 *  TRINETRA — One-Click AI Overview
...
async function geminiOverview(mode: Mode, digest: Digest, keys: string[]): Promise<string | null> {
  try {
    ...
    const model = client.getGenerativeModel({
      ...
      systemInstruction:
        'You are TRINETRA, a terse intelligence analyst. Given structured facts, write a sharp 2-4 sentence situational read-out. No preamble, no markdown headers, no hedging. Lead with the bottom line.',
    });
    ...
    console.warn('[TRINETRA] Gemini overview failed, using heuristic:', e);
    return null;
  }
}
