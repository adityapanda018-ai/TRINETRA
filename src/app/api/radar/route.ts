...
/**
 * TRINETRA — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'TRINETRA/4.2', 'Accept': 'application/json' },
    });

    console.log('[TRINETRA] IODA response status:', res.status);
    ...
    console.error('[TRINETRA] IODA fetch error:', error);
