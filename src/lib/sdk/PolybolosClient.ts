 ...
 * ║                                                                 ║
 * ║  Subscribes to TRINETRA feeds + external providers (Lattice),   ║
...
// ── TRINETRA Feed → Entity Translators ──────────────────────────────
...
function translateFlights(flights: any[], subtype: string): PolybolosEntity[] {
  ...
  return flights.map((f: any) => ({
    id: `trinetra-air-${f.icao24 || f.callsign || Math.random().toString(36).slice(2)}`,
    ...
    source: { provider: 'trinetra', feed: `flights-${subtype}`, originalId: f.icao24, confidence: 0.9 },
...
function translateMaritime(ships: any[]): PolybolosEntity[] {
  ...
  return ships.map((s: any) => ({
    id: `trinetra-sea-${s.mmsi || s.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'trinetra', feed: 'maritime-ais', originalId: s.mmsi?.toString(), confidence: 0.85 },
    timestamp: new Date().toISOString(),
...
function translateEarthquakes(events: any[]): PolybolosEntity[] {
  ...
  return events.map((eq: any) => ({
    id: `trinetra-event-eq-${eq.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'trinetra', feed: 'usgs-earthquakes', originalId: eq.id, confidence: 0.99 },
    timestamp: new Date().toISOString(),
...
function translateSatellites(sats: any[]): PolybolosEntity[] {
  ...
  return sats.map((s: any) => ({
    id: `trinetra-space-${s.noradId || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'trinetra', feed: 'satnogs', originalId: s.noradId?.toString(), confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
function translateFires(fires: any[]): PolybolosEntity[] {
  ...
  return fires.map((f: any, i: number) => ({
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'trinetra', feed: 'nasa-firms', confidence: 0.9 },
    timestamp: new Date().toISOString(),
...
function translateCCTV(cameras: any[]): PolybolosEntity[] {
  ...
  return cameras.map((c: any) => ({
    id: `trinetra-sensor-cctv-${c.id || Math.random().toString(36).slice(2)}`,
    name: c.name || 'Camera',
    domain: Domain.LAND,
    ...
    threat: ThreatLevel.NONE,
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'trinetra', feed: 'cctv-network', originalId: c.id, confidence: 1.0 },
    timestamp: new Date().toISOString(),
...
function translateRadiation(stations: any[]): PolybolosEntity[] {
  ...
  return stations.map((r: any) => ({
    id: `trinetra-sensor-rad-${r.name || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'trinetra', feed: 'radiation-network', confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
export class PolybolosClient {
  ...
  /** Connect to the TRINETRA SSE stream endpoint */
  private connectSSE(): void {
    if (typeof EventSource === 'undefined') return;

    try {
      const baseUrl = this.config.trinetraBaseUrl || this.config.osirisBaseUrl || '';
      this.sseConnection = new EventSource(
        `${baseUrl}/api/sdk/stream`
      );
  ...
  /**
   * Ingest raw TRINETRA data and translate it into Polybolos entities.
   * This is the primary method called by page.tsx to feed data into the SDK.
   */
  ingestTrinetraData(data: Record<string, any>): void {
  ...
  /** Legacy alias for ingestTrinetraData */
  ingestOsirisData(data: Record<string, any>): void {
    this.ingestTrinetraData(data);
  }
  ...
  private getActiveFeedCount(): number {
    let count = 0;
    // Count TRINETRA feeds that have data
